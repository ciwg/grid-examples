...
func Run(ctx context.Context, cfg agent.Config) (runErr error) {
	...
	for {
		...
		if err := agent.VerifyMessageCapability(request.CapabilityToken, "seller", "accounting", protocol.AccountingProfile, request.Kind); err != nil {
			if observationErr := client.RecordObservation(artifact.ObservationRecord{Kind: "invalid_capability", RawCID: requestCID, ObservedPCID: protocol.AccountingProfile.CID.String(), Reason: err.Error()}); observationErr != nil {
