...
Status: active
Decision: Implement the first context package as a first-party built-in egg while the installed-package mutation contract remains too shallow for serious durable writes.
Intent: Get the first real ex6 package into code without pretending the external package ABI is further along than it is.
Constraints: Keep the package under `packages/context/`; preserve the basket-first runtime contract; do not bypass durable history.
