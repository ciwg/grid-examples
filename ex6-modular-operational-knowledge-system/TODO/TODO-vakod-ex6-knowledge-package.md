...
Status: active
Decision: Implement the first knowledge package as a first-party built-in package with item, revision, and lifecycle families while the installed-package mutation contract remains shallow.
...
Constraints: Keep the package under `packages/knowledge/`; keep revision bodies CAS-backed; preserve durable history writes.
