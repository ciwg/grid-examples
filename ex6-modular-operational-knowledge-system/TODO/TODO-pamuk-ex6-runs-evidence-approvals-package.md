...
Status: active
Decision: Implement the first runs package as a first-party built-in package with run, evidence, and approval families while the installed-package mutation contract remains shallow.
Intent: Get the durable performed-work surface into ex6 code now so the runtime has real operational records beyond context and document state.
