...
- `moks route plan <protocol-pcid>` returns the kernel's preferred executable route plan plus ordered candidates, winner explanation, and pairwise candidate comparisons under the active policy layers
...
Source: `DI-rutom`; `DI-ruvot`; `DI-lafek`; `DI-fotav`; `DI-pabut`; `DI-matek`; `DI-posek`; `DI-rivuk`; `DI-lavik`; `DI-fobek`.
