...
- `moks route plan <protocol-pcid>` returns the kernel's preferred executable route plan plus ordered candidates and an explanation of why the winning route outranked the next candidate under the active policy layers
...
Source: `DI-rutom`; `DI-ruvot`; `DI-lafek`; `DI-fotav`; `DI-pabut`; `DI-matek`; `DI-posek`; `DI-rivuk`; `DI-lavik`.
