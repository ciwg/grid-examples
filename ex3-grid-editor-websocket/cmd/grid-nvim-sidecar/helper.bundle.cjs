...
// node_modules/@automerge/automerge/dist/mjs/wasm_bindgen_output/nodejs/automerge_wasm.cjs
var require_automerge_wasm = __commonJS({
  "node_modules/@automerge/automerge/dist/mjs/wasm_bindgen_output/nodejs/automerge_wasm.cjs"(exports2, module2) {
    var imports = {};
...
${val.stack}`;
    ...
    module2.exports.importSyncState = function(state2) {
      try {
        const retptr = wasm2.__wbindgen_add_to_stack_pointer(-16);
        wasm2.importSyncState(retptr, addHeapObject2(state2));
        var r0 = getDataViewMemory02().getInt32(retptr + 4 * 0, true);
        var r1 = getDataViewMemory02().getInt32(retptr + 4 * 1, true);
        var r2 = getDataViewMemory02().getInt32(retptr + 4 * 2, true);
        if (r2) {
    ...
    module2.exports.exportSyncState = function(state2) {
    ...
    module2.exports.encodeSyncState = function(state2) {
      _assertClass2(state2, SyncState2);
    ...
    var Automerge2 = class _Automerge {
      ...
      receiveSyncMessage(state2, message) {
        try {
          const retptr = wasm2.__wbindgen_add_to_stack_pointer(-16);
          _assertClass2(state2, SyncState2);
          wasm2.automerge_receiveSyncMessage(retptr, this.__wbg_ptr, state2.__wbg_ptr, addHeapObject2(message));
          var r0 = getDataViewMemory02().getInt32(retptr + 4 * 0, true);
          var r1 = getDataViewMemory02().getInt32(retptr + 4 * 1, true);
          if (r1) {
            throw takeObject2(r0);
...
function importSyncState(state2) {
  try {
    const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
    wasm.importSyncState(retptr, addHeapObject(state2));
    var r0 = getDataViewMemory0().getInt32(retptr + 4 * 0, true);
    var r1 = getDataViewMemory0().getInt32(retptr + 4 * 1, true);
    var r2 = getDataViewMemory0().getInt32(retptr + 4 * 2, true);
    if (r2) {
...
function exportSyncState(state2) {
  ...
  const ret = wasm.exportSyncState(state2.__wbg_ptr);
...
function encodeSyncState(state2) {
  _assertClass(state2, SyncState);
  const ret = wasm.encodeSyncState(state2.__wbg_ptr);
  return takeObject(ret);
}
...
var Automerge = class _Automerge {
  ...
  receiveSyncMessage(state2, message) {
    try {
      const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
      _assertClass(state2, SyncState);
      wasm.automerge_receiveSyncMessage(retptr, this.__wbg_ptr, state2.__wbg_ptr, addHeapObject(message));
      var r0 = getDataViewMemory0().getInt32(retptr + 4 * 0, true);
      var r1 = getDataViewMemory0().getInt32(retptr + 4 * 1, true);
      if (r1) {
        throw takeObject(r0);
...
// node_modules/@automerge/automerge/dist/mjs/entrypoints/fullfat_node.js
...
function view(doc, heads) {
  ...
  return state2.handle.materialize("/", heads, Object.assign(Object.assign({}, state2), {
    handle,
    heads
  }));
}
function clone(doc, _opts) {
  const state2 = _state(doc);
  const heads = state2.heads;
  const opts = importOpts(_opts);
  const handle = state2.handle.fork(opts.actor, heads);
  handle.updateDiffCursor();
  const { heads: _oldHeads } = state2, stateSansHeads = __rest(state2, ["heads"]);
...
function _change(doc, source, options, callback, scope) {
  ...
  try {
    state2.heads = heads;
    const root = rootProxy(state2.handle, state2.textV2);
    callback(root);
...
function emptyChange(doc, options) {
  ...
  state2.handle.emptyChange(options.message, options.time);
...
function saveIncremental(doc) {
  ...
  return state2.handle.saveIncremental();
}
function save(doc) {
...
function getActorId(doc) {
  const state2 = _state(doc);
  return state2.handle.getActorId();
}
function getLastLocalChange(doc) {
  const state2 = _state(doc);
  return state2.handle.getLastLocalChange() || void 0;
}
function getObjectId(doc, prop) {
  if (prop) {
    const state2 = _state(doc, false);
    const objectId = _obj(doc);
    if (!state2 || !objectId) {
      return null;
    }
    return state2.handle.get(objectId, prop);
  } else {
    return _obj(doc);
  }
}
...
function getAllChanges(doc) {
  const state2 = _state(doc);
  return state2.handle.getChanges([]);
}
function applyChanges(doc, changes, opts) {
  ...
  const heads = state2.handle.getHeads();
  state2.handle.applyChanges(changes);
  state2.heads = heads;
  return [
    progressDocument(doc, "applyChanges", heads, opts.patchCallback || state2.patchCallback)
  ];
}
function getHistory(doc) {
  ...
  return history.map((change2, index) => ({
    ...
    get snapshot() {
      const [state2] = applyChanges(init({ enableTextV2: textV2 }), history.slice(0, index + 1));
      return state2;
    }
  }));
}
function diff(doc, before, after) {
  ...
  const state2 = _state(doc);
  if (state2.mostRecentPatch && equals(state2.mostRecentPatch.before, before) && equals(state2.mostRecentPatch.after, after)) {
    return state2.mostRecentPatch.patches;
  }
  return state2.handle.diff(before, after);
}
...
function encodeSyncState2(state2) {
  const sync = ApiHandler.importSyncState(state2);
  const result = ApiHandler.encodeSyncState(sync);
  sync.free();
  return result;
}
function decodeSyncState2(state2) {
  const sync = ApiHandler.decodeSyncState(state2);
...
function hasOurChanges(doc, remoteState) {
  ...
  return state2.handle.hasOurChanges(syncState);
}
...
function getMissingDeps(doc, heads) {
  const state2 = _state(doc);
  return state2.handle.getMissingDeps(heads);
}
function getHeads(doc) {
  const state2 = _state(doc);
  return state2.heads || state2.handle.getHeads();
}
function dump(doc) {
  const state2 = _state(doc);
  state2.handle.dump();
...
function toJS(doc) {
  const state2 = _state(doc);
  const enabled = state2.handle.enableFreeze(false);
  const result = state2.handle.materialize("/", state2.heads);
  state2.handle.enableFreeze(enabled);
  return result;
}
...
function saveSince(doc, heads) {
  const state2 = _state(doc);
  const result = state2.handle.saveSince(heads);
  return result;
}
function hasHeads(doc, heads) {
  const state2 = _state(doc);
  for (const hash of heads) {
    if (!state2.handle.getChangeByHash(hash)) {
      return false;
    }
  }
  return true;
...
function topoHistoryTraversal(doc) {
  const state2 = _state(doc);
  return state2.handle.topoHistoryTraversal();
}
function inspectChange(doc, changeHash) {
  const state2 = _state(doc);
  return state2.handle.getDecodedChangeByHash(changeHash);
}
function stats(doc) {
  const state2 = _state(doc);
  return state2.handle.stats();
}
...
function cursorToIndex(state2, value, index) {
  if (typeof index == "string") {
    if (/^-?[0-9]+@[0-9a-zA-Z]+$|^[se]$/.test(index)) {
      return state2.handle.getCursorPosition(value, index);
    } else {
...
function splice(doc, path, index, del, newText) {
  ...
  const state2 = _state(doc, false);
  _clear_cache(doc);
  index = cursorToIndex(state2, objPath, index);
  try {
    return state2.handle.splice(objPath, index, del, newText);
  } catch (e) {
...
function updateText(doc, path, newText) {
  ...
  _clear_cache(doc);
  try {
    return state2.handle.updateText(objPath, newText);
  } catch (e) {
...
function spans(doc, path) {
  ...
  try {
    return state2.handle.spans(objPath, state2.heads);
  } catch (e) {
    throw new RangeError(`Cannot splice: ${e}`);
  }
}
function block(doc, path, index) {
  ...
  index = cursorToIndex(state2, objPath, index);
  try {
    return state2.handle.getBlock(objPath, index);
  } catch (e) {
...
function splitBlock(doc, path, index, block2) {
  ...
  index = cursorToIndex(state2, objPath, index);
  try {
    state2.handle.splitBlock(objPath, index, block2);
  } catch (e) {
    throw new RangeError(`Cannot splice: ${e}`);
  }
}
function joinBlock(doc, path, index) {
  ...
  index = cursorToIndex(state2, objPath, index);
  try {
    state2.handle.joinBlock(objPath, index);
  } catch (e) {
...
function updateBlock(doc, path, index, block2) {
  ...
  index = cursorToIndex(state2, objPath, index);
  try {
    state2.handle.updateBlock(objPath, index, block2);
  } catch (e) {
...
function updateSpans(doc, path, newSpans) {
  ...
  _clear_cache(doc);
  try {
    state2.handle.updateSpans(objPath, newSpans);
  } catch (e) {
    throw new RangeError(`Cannot updateBlock: ${e}`);
  }
}
function getCursor(doc, path, position, move) {
  ...
  const state2 = _state(doc, false);
  try {
    return state2.handle.getCursor(objPath, position, state2.heads, move);
  } catch (e) {
...
function getCursorPosition(doc, path, cursor) {
  ...
  const state2 = _state(doc, false);
  try {
    return state2.handle.getCursorPosition(objPath, cursor, state2.heads);
  } catch (e) {
...
function mark(doc, path, range, name, value) {
  ...
  const state2 = _state(doc, false);
  try {
    return state2.handle.mark(objPath, range, name, value);
  } catch (e) {
...
function unmark(doc, path, range, name) {
  ...
  const state2 = _state(doc, false);
  try {
    return state2.handle.unmark(objPath, range, name);
  } catch (e) {
...
function marks(doc, path) {
  ...
  const state2 = _state(doc, false);
  try {
    return state2.handle.marks(objPath);
  } catch (e) {
...
function marksAt(doc, path, index) {
  ...
  const state2 = _state(doc, false);
  try {
    return state2.handle.marksAt(objPath, index);
  } catch (e) {
...
function getConflicts(doc, prop) {
  ...
  const objectId = _obj(doc);
  if (objectId != null) {
    return unstableConflictAt(state2.handle, objectId, prop);
  } else {
    return void 0;
  }
}
...
var pendingMessage = Promise.resolve();
rl.on("line", (line) => {
  if (!line.trim()) {
    return;
  }
...
function applyLocalText(content) {
  ...
  state.doc = change(clone(state.doc), (draft) => {
    AutomergeNext.splice(draft, ["content"], prefix, deleteCount, insertText);
  });
  const change2 = getLastLocalChange(state.doc);
  if (change2) {
    postChange(change2).catch((error) => send({ type: "error", message: error.stack || error.message }));
  }
}
...
async function postChange(changeBytes) {
  ...
  const replicaBase64 = bytesToBase64(save(state.doc));
...
async function receive(record) {
  ...
  const [nextDoc] = applyChanges(state.doc, [base64ToBytes(record.message_base64)]);
...
async function hydrateFromSnapshot() {
  ...
  state.doc = ensureDocument(load2(base64ToBytes(snapshot.replica_base64)));
