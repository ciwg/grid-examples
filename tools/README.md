...
This directory owns the repository-wide allocator. Source: DI-nujof.
...
serializes allocation through the root `TODO/handle-namespace.tsv` ledger and
checks all existing repository identifiers, so concurrent agents cannot
allocate the same new handle.
