...
Status: active
Decision: Extend the installed-package contract so `run` may return runtime-mediated CAS write requests and record append requests instead of limiting installed packages to plain text output.
Intent: Let outside package authors create durable records through the runtime without bypassing the runtime or waiting for a later full plugin ABI.
...
Intent: Make installed executable packages behave like real installed packages instead of process-local registrations that disappear on the next command.
...
The product is not genuinely modular if only the repo author can add packages.
