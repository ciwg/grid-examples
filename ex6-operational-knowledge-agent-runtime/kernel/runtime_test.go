...
func TestProtocolRoutePlanTraceCanUseNamedScopes(t *testing.T) {
	runtimeRoot := filepath.Join(t.TempDir(), ".moks")
	runtime, err := kernel.Open(runtimeRoot)
	if err != nil {
		t.Fatalf("open runtime: %v", err)
	...
	nested := kernel.BuiltinPackage{
		Manifest: pkgmeta.Manifest{
			...
			Claims: []pkgmeta.ImplementationClaim{
				{
					ProtocolPCID:   "pcid:moks.scope.root.v1",
					Role:           "parser",
					RouteType:      "parser",
					EmitsProtocols: []string{"pcid:moks.scope.mid.v1"},
					Summary:        "Root parser.",
				},
				{
					ProtocolPCID:   "pcid:moks.scope.mid.v1",
					Role:           "parser",
					RouteType:      "parser",
					EmitsProtocols: []string{"pcid:moks.scope.leaf.v1"},
					Summary:        "Mid parser.",
				},
				{
					ProtocolPCID: "pcid:moks.scope.leaf.v1",
					Role:         "handler",
					Summary:      "Leaf handler.",
				},
			},
	...
	if err := runtime.RegisterBuiltin(nested); err != nil {
		t.Fatalf("register scope parser: %v", err)
	}
	direct := runtime.ProtocolRoutePlanTraceFocused("pcid:moks.scope.root.v1", kernel.RoutePlanTraceFilter{
		Clauses: []kernel.RoutePlanTraceFilterClause{
			{Kind: "scope", Target: "direct-hops"},
		},
	})
	if len(direct.Explanation.DownstreamTraceSummaries) != 1 || direct.Explanation.DownstreamTraceSummaries[0].HopDepth != 1 {
		t.Fatalf("expected direct-hops scope to keep only depth-1 summaries, got %#v", direct.Explanation.DownstreamTraceSummaries)
	}
	deep := runtime.ProtocolRoutePlanTraceFocused("pcid:moks.scope.root.v1", kernel.RoutePlanTraceFilter{
		Clauses: []kernel.RoutePlanTraceFilterClause{
			{Kind: "scope", Target: "deep-hops"},
		},
	})
	if len(deep.Explanation.DownstreamTraceSummaries) != 1 || deep.Explanation.DownstreamTraceSummaries[0].HopDepth != 2 {
		t.Fatalf("expected deep-hops scope to keep only depth-2+ summaries, got %#v", deep.Explanation.DownstreamTraceSummaries)
	}
	branch := runtime.ProtocolRoutePlanTraceFocused("pcid:moks.scope.root.v1", kernel.RoutePlanTraceFilter{
		Clauses: []kernel.RoutePlanTraceFilterClause{
			{Kind: "scope", Target: "one-branch:scope-parser:parser:parser"},
		},
	})
	if len(branch.Explanation.DownstreamTraceSummaries) == 0 {
		t.Fatalf("expected one-branch scope to keep branch summaries, got %#v", branch.Explanation.DownstreamTraceSummaries)
	}
}
...
func TestInspectTraceScopeWithQueryOrdersAndFiltersGroups(t *testing.T) {
	...
	if inspection.QuerySummary == nil || inspection.QuerySummary.TotalGroups != 3 || inspection.QuerySummary.MatchedGroups != 2 || inspection.QuerySummary.HiddenGroups != 1 || inspection.QuerySummary.Ordering != "summary" {
		t.Fatalf("expected query summary, got %#v", inspection.QuerySummary)
	}
	...
	if depthFiltered.QuerySummary == nil || depthFiltered.QuerySummary.TotalGroups != 3 || depthFiltered.QuerySummary.MatchedGroups != 1 || depthFiltered.QuerySummary.HiddenGroups != 2 || depthFiltered.QuerySummary.Ordering != "label" {
		t.Fatalf("expected default-order query summary, got %#v", depthFiltered.QuerySummary)
	}
	if depthFiltered.QueryDiagnostics == nil || !depthFiltered.QueryDiagnostics.DefaultOrderingApplied || depthFiltered.QueryDiagnostics.DefaultOrderingReason != "no sort provided; defaulted to label ordering" || depthFiltered.QueryDiagnostics.ZeroMatches {
		t.Fatalf("expected default-order diagnostics, got %#v", depthFiltered.QueryDiagnostics)
	}
	zeroFiltered, ok := runtime.InspectTraceScopeWithQuery("ordered-scope", kernel.RouteScopeGroupQuery{
		DepthFilter: "9",
	})
	if !ok || len(zeroFiltered.Groups) != 0 {
		t.Fatalf("expected zero depth-filtered branches, got %#v %#v", ok, zeroFiltered.Groups)
	}
	if zeroFiltered.QuerySummary == nil || zeroFiltered.QuerySummary.TotalGroups != 3 || zeroFiltered.QuerySummary.MatchedGroups != 0 || zeroFiltered.QuerySummary.HiddenGroups != 3 || zeroFiltered.QuerySummary.Ordering != "label" {
		t.Fatalf("expected zero-match query summary, got %#v", zeroFiltered.QuerySummary)
	}
