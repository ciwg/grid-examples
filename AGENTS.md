...
Once a TE is filed in `docs/thought-experiments/`, edits to it follow the categorized policy below. The formerly cited source TEs `TE-dabol` and `TE-vudaf` are absent from every recoverable repository source and must not be recreated as fabricated history. This self-contained policy repair preserves the existing category rules and holistic-reading default. Source: DI-surob; TE-nuvib.
...
Reading default: holistic. When deciding whether an edit is mechanical or substantive, when interpreting a single TE's claims, or when reasoning about whether a refinement is Cat-3 or Cat-5–7, the agent must read the affected TE plus every extant TE that it cites or that cites it, and any available policy TE named by those records. Single-TE reading is reserved for obviously mechanical questions (a single typo; a path that has demonstrably moved; a Status field retrofit) and only after the available corpus confirms the question is mechanical. When in doubt, read holistically. Source: DI-surob; TE-nuvib.
