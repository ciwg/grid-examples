...
`../grid-examples-browser-checks/ex5/`. It must attach to a prelaunched,
verified Google Chrome session over DevTools; it must not launch a synthetic
browser session. Its assertions cover Draft queue, Problem hotspots,
...
session. Source: `DI-sobek`; `DI-punek`; `DI-bahak`; `DI-danir`.
